...
int main(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "c:Vv")) != EOF) {
		...
		case 'c': {
			...
			if (errno) {
				fprintf(stderr, "%s:  Invalid policyvers specified: %s\n", argv[0], optarg);
				usage(argv[0]);
				exit(1);
			}
			if (n < sepol_policy_kern_vers_min() || n > sepol_policy_kern_vers_max()) {
				fprintf(stderr, "%s:  policyvers value %ld not in range %d-%d\n",
					argv[0], n, sepol_policy_kern_vers_min(), sepol_policy_kern_vers_max());
				usage(argv[0]);
				exit(1);
			}
			policyvers = n;
	...
	if (verbose) {
        ...
        if (argc < 3 || !(optind != (argc - 1))) {
                fprintf(stderr, "%s:  You must provide the base module package and output filename\n", argv[0]);
                usage(argv[0]);
        }
	...
	if (ret) {
		fprintf(stderr, "%s:  Error while writing expanded policy to %s\n",
			argv[0], outname);
		exit(1);
	}
	fclose(outfile);
