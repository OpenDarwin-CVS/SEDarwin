...
hidden int semanage_node_compare2_qsort(
	const semanage_node_t** node,
	const semanage_node_t** node2) {
...
int semanage_node_key_create(
	...
	return sepol_node_key_create(handle->sepolh, addr, mask, proto, key_ptr);
}

int semanage_node_key_extract(
	semanage_handle_t* handle,
	const semanage_node_t* node,
	semanage_node_key_t** key_ptr) {
...
int semanage_node_get_addr_bytes(
	semanage_handle_t* handle,
	const semanage_node_t* node,
	char** addr,
	size_t* addr_sz) {	
...
int semanage_node_get_mask_bytes(
	semanage_handle_t* handle,
	const semanage_node_t* node,
	char** mask,
	size_t* mask_sz) {
...
int semanage_node_set_mask_bytes(
	semanage_handle_t* handle,
	semanage_node_t* node,
	const char* mask,
	size_t mask_sz) {
...
const char* semanage_node_get_proto_str(
	int proto) {
...
/* Context */
semanage_context_t* semanage_node_get_con(
	const semanage_node_t* node) {
