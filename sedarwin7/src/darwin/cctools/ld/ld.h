...
static inline kern_return_t rkld_alloc (mach_port_t kport, size_t size, void **addr, int flags)
{
  if (kth_started)
    ...
    {
      /*
       * XXXMAC: We replace the allocator here to see if we can avoid
       * pmap collision panics, but we may wish to re-evaluate this
       * change and revert to kalloc.  Likewise just below rkld_free,
       * we replace kfree with kmem_free.
       */
      //*addr = kalloc (size);
      //return (*addr == 0);
      return kmem_alloc_wired (kernel_map, addr, size);
    }
}
...
/*
 * XXXMAC: To be safe, we are temporarily leaking linker memory, this
 * needs to be re-evaluated and corrected.
 */
#define vm_deallocate(kport,addr,size) /*rkld_free (kport, size, addr)*/0
