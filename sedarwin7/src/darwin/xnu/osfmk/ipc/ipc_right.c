	...
	mach_msg_type_name_t	msgt_name)
{
	...
	switch (msgt_name) {
		...
		switch (msgt_name) {
		case MACH_MSG_TYPE_COPY_SEND:
		    rc = mac_check_port_copy_send(&self->maclabel,
			&port->ip_label);
		    break;
		case MACH_MSG_TYPE_MOVE_SEND:
		    rc = mac_check_port_move_send(&self->maclabel,
			&port->ip_label);
		    break;
		case MACH_MSG_TYPE_MOVE_SEND_ONCE:
		    rc = mac_check_port_move_send_once(&self->maclabel,
			&port->ip_label);
		    break;
		default:
...
#ifdef MAC
		tasklabel_lock(self);
		rc = mac_check_port_move_receive(&self->maclabel,
						 &port->ip_label);
		tasklabel_unlock(self);
		if (rc) {
			ip_unlock(port);
	...
	ipc_object_t		object)
{
	...
	switch (msgt_name) {
		...
		if (space->is_task) {
			...
			rc = mac_check_port_hold_send_once(&space->is_task->maclabel,
						      &port->ip_label);
		...
		if (space->is_task) {
			tasklabel_lock(space->is_task);
			rc = mac_check_port_hold_send(&space->is_task->maclabel,
						      &port->ip_label);
			tasklabel_unlock(space->is_task);
		...
		if (space->is_task) {
			tasklabel_lock(space->is_task);
			rc = mac_check_port_hold_receive(&space->is_task->maclabel,
							 &port->ip_label);
			tasklabel_unlock(space->is_task);
