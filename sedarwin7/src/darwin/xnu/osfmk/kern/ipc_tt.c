...
#ifdef MAC
	if (parent)
		mac_create_task(parent, task, &parent->maclabel,
		    &task->maclabel, &kport->ip_label);
	else
		mac_create_kernel_task(task, &task->maclabel, &kport->ip_label);
#endif
...
#ifdef MAC
	if (mac_check_task_service_access (current_task(), task, "set_special_port"))
