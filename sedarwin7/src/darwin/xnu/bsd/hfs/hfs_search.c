...
static int CheckAccess(ExtendedVCB *vcb, u_long searchBits, CatalogKey *key,
		       CatalogRecord *rec, struct proc *p);
	...
	*/
{
	...
	*(ap->a_nummatches) = 0;
	...
	/* 
	 * Unlock the caller's vnode. We only use it for the v_mount, which is assumed
	 * to be safe. The caller's reference is retained to prevent the vnode from
	 * being reused during this call. The vnode is relocked before returning to
	 * preserve the original calling convention.
	 */
	VOP_UNLOCK(ap->a_vp, 0, ap->a_uio->uio_procp);
			 ...
			 IsTargetName( &searchInfo1, isHFSPlus )  )
		{
			...
			if ( result == E_NONE ) {
				if (CheckCriteria(vcb, ap->a_options, ap->a_searchattrs, &rec,
				        ...
				        CheckAccess(vcb, ap->a_options, keyp, &rec, ap->a_uio->uio_procp)) {
	...
	for (;;) {
		...
		&&  CheckAccess(vcb, ap->a_options, myCurrentKeyPtr, myCurrentDataPtr,
				ap->a_uio->uio_procp)) {
...
ExitThisRoutine:
	...
	vn_lock(ap->a_vp,  LK_EXCLUSIVE|LK_RETRY, ap->a_uio->uio_procp);
...
static int
CheckAccess(ExtendedVCB *theVCBPtr, u_long searchBits, CatalogKey *theKeyPtr, CatalogRecord *rec,
	    struct proc *theProcPtr)
{
...
#ifdef MAC
	/*
	 * Check only extensible permissions on the object itself;
	 * unix permission checks only need to examine the containing directories.
	 */

	switch (rec->recordType) {
	case kHFSFolderRecord:
		myNodeID = rec->hfsFolder.folderID;
		break;

	case kHFSFileRecord:
		myNodeID = rec->hfsFile.fileID;
		break;

	case kHFSPlusFolderRecord:
		myNodeID = rec->hfsPlusFolder.folderID;
		break;

	case kHFSPlusFileRecord:
		myNodeID = rec->hfsPlusFile.fileID;
		break;

	default:
		printf ("hfs_search: warning: unknown catalog record type %d\n",
			rec->recordType);
		return 0;
	}
#else	   
	...
	while ( myNodeID >= kRootDirID ) {
		...
		myErr = hfs_getcnode(my_hfsmountPtr, myNodeID, NULL, 0, NULL, NULL, &vp);
		if ( myErr ) {
			goto ExitThisRoutine;   /* no access */
		}

		my_cat_attr = VTOC(vp)->c_attr;
		my_cat_desc = VTOC(vp)->c_desc;

		if (check_readdir)
			myErr = mac_check_vnode_readdir(theProcPtr->p_ucred, vp);
		else
			myErr = mac_check_vnode_stat(theProcPtr->p_ucred, NOCRED, vp);
		if ( myErr ) {
			goto ExitThisRoutine;   /* no access */
		}
#else
...
#ifdef MAC
		if (!check_readdir) {
			check_readdir = 1;  /* need readdir permission on parent directories */
			continue;           /* skip unix permission checks on object itself */
		}
#endif
...
ExitThisRoutine:
	...
	if (vp)
		vput (vp);
#else
	/* Since we use the cat_desc from the cnode, it should not be freed. */
	cat_releasedesc( &my_cat_desc );
#endif
	return ( myResult );
