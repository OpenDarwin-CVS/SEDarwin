...
#ifdef MAC
	/*-
	 * XXXMAC: This code differs from FreeBSD in several important
	 * ways, which may require attention in the future:
	 *
	 * - FreeBSD checks the return value of this call, and if it
	 *   fails, then we return the vnode for recycling and and
	 *   return an error to the caller.  This means we fail stop
	 *   access to vnodes that we cannot retrieve the label for
	 *   due to an error (disk or otherwise).
	 * - FreeBSD performs the associate operation only if the file
	 *   system is multi-label, relying on the the VFS framework
	 *   to assign labels on single-label file systems.
	 */
	(void) mac_associate_vnode_extattr (hfsmp->hfs_mp, vp);
#endif
	*vpp = vp;
	return (0);
}
