...
hf_insert(hotfile_data_t *hotdata, hotfile_entry_t *newentry) 
{
	...
	while (entry) {
		/*
		 * We've already built the tree as deep as we can go.  This
		 * protects against unbounded recursion in hf_sortlist()
		 *
		 * XXX Better solution is to remove the coldest entry and
		 * rebalance the tree.
		 */
		if (entry->depth == HFC_BTREE_MAXDEPTH) {
			newentry->right = hotdata->freelist; 
			hotdata->freelist = newentry; 		
			return;
		}
