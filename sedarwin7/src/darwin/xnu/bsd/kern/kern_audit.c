...
audit_free(struct kaudit_record *ar)
{
	...
	if (ar->k_ar.ar_vnode1_mac_labels != NULL) {
		kfree((vm_offset_t) ar->k_ar.ar_vnode1_mac_labels,
			  MAC_AUDIT_LABEL_LEN);
	}
	if (ar->k_ar.ar_vnode2_mac_labels != NULL) {
		kfree((vm_offset_t) ar->k_ar.ar_vnode2_mac_labels,
			  MAC_AUDIT_LABEL_LEN);
	}
	if (ar->k_ar.ar_cred_mac_labels != NULL) {
		kfree((vm_offset_t) ar->k_ar.ar_cred_mac_labels,
			  MAC_AUDIT_LABEL_LEN);
	}
	...
	do {
		...
		while (head != NULL) {
			...
			kfree((vm_offset_t) head->data, head->length);
			kfree((vm_offset_t) head, sizeof(*head));
			head = next;
		}

		kfree((vm_offset_t) ar->k_ar.ar_mac_records,
...
#ifdef MAC
        error = mac_check_system_audit(pc->pc_ucred, rec, uap->length);
	if (error)
		goto free_out;
#endif
...
#ifdef MAC
	ret = mac_check_system_auditon(pc->pc_ucred, uap->cmd);
	if (ret)
		return (ret);
#endif
...
#ifdef MAC
	error = mac_check_proc_getauid(pc->pc_ucred);
	if (error)
		return (error);
#endif
...
#ifdef MAC
	/*  
	 *  Copy the parameter into a temp var first to avoid making changes
	 *  before the MAC check.
	 */
	error = copyin((void *) uap->auid, (void *) &temp,
	    sizeof(p->p_au->ai_auid));
	if (error)
		return (error);
		
	error = mac_check_proc_setauid(pc->pc_ucred, temp);
	if (error)
		return (error);
...
#ifdef MAC
	error = mac_check_proc_getaudit(pc->pc_ucred);
	if (error)
		return (error);
#endif
...
setaudit(struct proc *p, struct setaudit_args *uap, register_t *retval)
{
	...
	/*  
	 * Copy the parameter into a temp var first to avoid making changes
	 * to the proc struct before the MAC check
	 */

	error = copyin((void *) uap->auditinfo, (void *) &temp, 
	    sizeof(*p->p_au));
	if (error)
		return (error);

	error = mac_check_proc_setaudit(pc->pc_ucred, &temp);
	if (error)
		return (error); 
...
auditctl(struct proc *p, struct auditctl_args *uap)
{
	...
	if (uap->path != NULL) {
		...
		error = mac_check_system_auditctl(p->p_ucred, nd.ni_vp);
#endif
		VOP_UNLOCK(nd.ni_vp, 0, p);
#ifdef MAC
		if (error) {
			vn_close(nd.ni_vp, audit_close_flags, p->p_ucred, p);
			goto out;
		}
#endif
		vp = nd.ni_vp;
		...
		error = mac_check_system_auditctl(p->p_ucred, NULL);
		if (error)
			return (error);
	}
#endif
 ...
 */
static void
audit_new_wait(int audit_event, struct proc *proc, struct uthread *uthread)
{
	int ret;
			...
			struct uthread *uthread)
{
	...
	if (audit_event != AUE_NULL) {
		...
		do {
			...
			error = mac_audit_preselect(proc->p_ucred, code,
			    (void *) uthread->uu_arg);

			/* 
			 * Note: The audit implementation in 10.3.3 is
			 * incomplete, so it was decided to give the
			 * MAC policies priority over the existing
			 * mechanism for now.  This means that a MAC
			 * policy can force an audit record to be
			 * suppressed even when the 10.3.3 mechanism
			 * would have allowed it.  This will probably
			 * change in the port to 10.3.4 where the
			 * audit implementation is more complete.
			 */
			if (error == MAC_AUDIT_YES) {
				uthread->uu_ar = audit_new(audit_event, proc,
				    uthread);
			    ...
			    au_preselect(audit_event, &proc->p_au->ai_mask,
...
#ifdef MAC
void
audit_syscall_exit(unsigned short code, int error, struct proc *proc, 
    struct uthread *uthread)
#else
void
...
#endif
{
	...
	do {
		 ...
		 * the port to 10.3.4.
		 */
		mac_error = mac_audit_postselect(proc->p_ucred, code,
		    (void *) uthread->uu_arg, error, retval,
		    uthread->uu_ar->k_ar.ar_forced_by_mac);

		if (mac_error == MAC_AUDIT_NO) {
 ...
 * 
 * XXX: It currently fails silently purely for the sake of being consistent
 * with the rest of the implementation.  This will be changed later,
 * in particular, to meet the requirements of "audit6".
 */
void
