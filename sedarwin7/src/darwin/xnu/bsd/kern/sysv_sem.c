	...
	int newSize;
{
...
#endif
	MALLOC(newSema, struct semid_kernel*, sizeof(struct semid_kernel)*newSize,
		M_SYSVSEM, M_WAITOK);
	if (NULL == newSema)
	{
#ifdef SEM_DEBUG
	...
	{
		newSema[i].u.sem_base = 0;
		newSema[i].u.sem_perm.mode = 0;
#ifdef MAC
		mac_init_sysv_sem(&newSema[i]);
#endif
	}
		...
		int newSize;
{
	...
	for(i = 0; i < seminfo.semmni; i++)
		...
		{
			if (newSem > sem)
				sema[i].u.sem_base += newSem - sem;
			else
				sema[i].u.sem_base -= sem - newSem;
		}
	...
	if (semid < 0 || semid >= seminfo.semmni)
{
	...
	if ((semakptr->u.sem_perm.mode & SEM_ALLOC) == 0 ||
		...
		UNLOCK_AND_RETURN(EINVAL);
#ifdef MAC
	if ((eval = mac_check_sysv_semctl(cred, semakptr, cmd))) {
		MPRINTF((
	...
	case IPC_STAT:
		...
		/*if ((eval = copyin(arg, &real_arg, sizeof(real_arg))) != 0)
			UNLOCK_AND_RETURN(eval);*/
		eval = copyout((caddr_t)&semakptr->u, arg.buf,
...
#ifdef MAC
			if ((eval = mac_check_sysv_semget(cred, &sema[semid]))) {
				MPRINTF((
	...
	register_t *retval;
{
	 ...
	 * sopptr->sem_op value.
	 */
	if ((eval = mac_check_sysv_semop(p->p_ucred, semakptr, j))) {
		MPRINTF((
	...
	for (;;) {
		...
		for (i = 0; i < nsops; i++) {
			...
			printf("semop:  semakptr=%x, sem_base=%x, semptr=%x, sem[%d]=%d : op=%d, flag=%s\n",
			    semkaptr, semkaptr->u.sem_base, semptr,
		...
		eval = tsleep((caddr_t)semakptr, (PZERO - 4) | PCATCH,
		    "semwait", 0);
                sysv_sem_sleeping_threads--;
...
sysctl_seminfo SYSCTL_HANDLER_ARGS
{
	...
	if ((sem == (struct sem *) 0) && 
		(sema == (struct semid_kernel *) 0) && 
		(semu == (struct sem_undo *) 0) && 
