...
{
  /* Structure to access the first load command in a mach-o file
     as a LC_SEGMENT. This function, and kload_linkedit_address 
     rely on the text segment command coming first in the object
     file, which is probably unsafe. In any case vendor updates
     should examine the use of mach-o headers in kload_linkedit_address
  ...
  */
  ...
  /*
   * XXXMAC: remove all possibilty that uninitialized values might
   * be used
   */
  entry->loaded_address = 0;
  ((struct machOMapping *) entry->object)->seg[0].vmaddr = 0;
 ...
 out:
  ...
  if (result) {
    result = __kload_start_module (entry);
    kmi->reference_count = 1;

    return result;
  }
  else
    return kload_error_link_load;
}
...
#else
	if (!kth_started) {
	    mach_result = kmem_alloc_wired (kernel_map, 
                    &G_current_load_entry->kernel_alloc_address,
